...
public:
  IndividualAveragePointsGnuplotFileWriter(
  ...
  void plot(
    ...
    if (found != data.cend()) {
      line("  \"" + found->second.string() + "\" u "
           + std::to_string(x_column()) + ":8 w lp lw "
...
class IndividualAveragePointsVsGameNumberGnuplotFileWriter
  : public IndividualAveragePointsGnuplotFileWriter {
public:
  IndividualAveragePointsVsGameNumberGnuplotFileWriter(
      const std::experimental::filesystem::path& path,
      const std::map<GameCategory, std::experimental::filesystem::path>&
          data) noexcept
    : IndividualAveragePointsGnuplotFileWriter(path) {
