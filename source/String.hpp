...
std::string pad_to_length(
    const std::string& text, const int64_t length) noexcept {
  std::string padded_text{text};
  if (length > static_cast<int64_t>(padded_text.size())) {
...
/// \brief Print a real number as a string to a given number of significant
/// digits.
std::string real_number_to_string(
    const double value, const int8_t significant_digits = 7) noexcept {
  if (value == 0.0) {
    return "0";
  } else {
    const double absolute_value{std::abs(value)};
    const double log10_absolute_value{std::log10(absolute_value)};
    const int64_t floor_log10_absolute_value{
        static_cast<int64_t>(std::floor(log10_absolute_value))};
    const int64_t raw_precision{static_cast<int64_t>(significant_digits)
                                - floor_log10_absolute_value - 1};
    const int64_t precision{static_cast<int64_t>(
        std::min(std::max(raw_precision, static_cast<int64_t>(0)),
