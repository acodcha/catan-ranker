...
std::string pad_to_length(const std::string& text, const int64_t length) noexcept {
...
std::string real_number_to_string(const double value, const int8_t significant_digits = 7) noexcept {
  if (value == 0.0) {
    return "0";
  } else {
    ...
    const int64_t precision{static_cast<int64_t>(std::min(std::max(raw_precision, static_cast<int64_t>(0)), static_cast<int64_t>(16)))};
