...
public:
  LeaderboardGlobalFileWriter(
    ...
    : MarkdownFileWriter(
        base_directory / Path::LeaderboardFileName, "Catan Leaderboard") {
    // Header.
    ...
    for (const GameCategory game_category : GameCategories) {
      ...
      list_link(
          label(game_category) + " " + subsection_title_average_points_plot_);
      list_link(label(game_category) + " "
                + subsection_title_first_place_percentage_plot_);
      ...
      subsection(
          label(game_category) + " " + subsection_title_elo_rating_plot_);
      ...
      link_back_to_section(label(game_category));
      subsection(label(game_category) + " "
                 + subsection_title_first_place_percentage_plot_);
    ...
    section("License");
    line(
        "This work is maintained by Alexandre Coderre-Chabot "
        "(https://github.com/acodcha) and licensed under the MIT License. For "
        "more details, see the `LICENSE` file or https://mit-license.org/. "
        "This work is based on the Catan board game (also known as Settlers of "
        "Catan) originally designed by Klaus Teuber. The contents, copyrights, "
        "and trademarks of everything involving Catan are exclusively held by "
        "its designers and publishers; I make no claim to any of these in any "
        "way.");
    link_back_to_top();
    blank_line();
  }
...
protected:
  ...
  void players_table(
      const Players& players, const GameCategory game_category) noexcept {
    bool is_empty{true};
    ...
    for (const Player& player : players) {
      ...
      if (latest.has_value()) {
        name.add_row("[" + player.name().value() + "]("
                     + player.name().directory_name().string() + ")");
    ...
    const Table data{
        {name, number_of_games, elo_rating, average_elo_rating,
         average_points_per_game, first_place, second_place, third_place}
    };
    table(data);
  }

  void elo_rating_plot(
      const std::experimental::filesystem::path& base_directory,
  ...
  void average_points_plot(
      const std::experimental::filesystem::path& base_directory,
      const GameCategory game_category) noexcept {
    const std::experimental::filesystem::path gnuplot_path{
        Path::MainPlotsDirectoryName
        / Path::global_average_points_vs_game_number_file_name(game_category)};
    if (std::experimental::filesystem::exists(base_directory / gnuplot_path)) {
  ...
  void place_percentage_plot(
    ...
    const std::experimental::filesystem::path gnuplot_path{
        ...
        / Path::global_place_percentage_vs_game_number_file_name(
            game_category, place)};
    if (std::experimental::filesystem::exists(base_directory / gnuplot_path)) {
      line("![" + place.print() + " Place History Plot]("
           + Path::gnuplot_path_to_png_path(gnuplot_path).string() + ")");
    }
  }

  void games_table(
      const Games& games, const GameCategory game_category) noexcept {
    bool is_empty{true};
