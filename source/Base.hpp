...
const std::map<uint8_t, GameCategory> game_categories{
...
GameCategory game_category(const uint8_t number_of_players) {
  const std::map<uint8_t, GameCategory>::const_iterator found_game_category{game_categories.find(number_of_players)};
