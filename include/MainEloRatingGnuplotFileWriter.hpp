...
public:
  ...
  MainEloRatingGnuplotFileWriter(const std::experimental::filesystem::path& path, const EloRating& peak) noexcept : GnuplotFileWriter(path) {
    const uint_least64_t y_maximum{std::max((uint_least64_t)(2 * EloRatingStartingValue), nearest_higher_nice_number(peak.value()))};
    ...
    line("set yrange [0:" + std::to_string(y_maximum) + "]");
    line("set ytics mirror out");
    //line("set mytics 10");
    line("set y2label \"\"");
    line("set y2range [0:" + std::to_string(y_maximum) + "]");
...
public:
  ...
  ) noexcept : MainEloRatingGnuplotFileWriter(path, players.peak_elo_rating(game_category)) {
