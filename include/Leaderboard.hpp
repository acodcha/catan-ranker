...
protected:
    ...
    for (const GameCategory game_category : GameCategories) {
      ...
      if (!data_paths.empty()) {
        ...
        MainAveragePointsVsDateGnuplotFileWriter{
          base_directory / Path::MainPlotsDirectoryName / Path::main_average_points_vs_date_file_name(game_category),
          data_paths
        };
        for (const Place& place : PlacesFirstSecondThird) {
          ...
          MainPlacePercentageVsDateGnuplotFileWriter{
            base_directory / Path::MainPlotsDirectoryName / Path::main_place_percentage_vs_date_file_name(game_category, place),
            data_paths, game_category, place
          };
        }
      }
    ...
    for (const Player& player : players) {
      ...
      if (!data_paths.empty()) {
        ...
        PlayerAveragePointsVsDateGnuplotFileWriter{
          base_directory / player.name().directory_name() / Path::PlayerPlotsDirectoryName / Path::PlayerAveragePointsVsDateFileName,
          data_paths
        };
      }
      for (const GameCategory game_category : GameCategories) {
        if (!player[game_category].empty()) {
          ...
          PlayerPlacePercentageVsDateGnuplotFileWriter{
            base_directory / player.name().directory_name() / Path::PlayerPlotsDirectoryName / Path::player_place_percentage_vs_date_file_name(game_category),
            base_directory / player.name().directory_name() / Path::PlayerDataDirectoryName / Path::player_data_file_name(game_category),
            game_category
          };
        }
  ...
  void generate_main_plots(const std::experimental::filesystem::path& base_directory) const {
    ...
    for (const GameCategory game_category : GameCategories) {
      ...
      generate_plot(base_directory / Path::MainPlotsDirectoryName / Path::main_average_points_vs_date_file_name(game_category));
      for (const Place& place : PlacesFirstSecondThird) {
        ...
        generate_plot(base_directory / Path::MainPlotsDirectoryName / Path::main_place_percentage_vs_date_file_name(game_category, place));
      }
    }
  ...
  void generate_player_plots(const std::experimental::filesystem::path& base_directory, const Players& players) const {
    ...
    for (const Player& player : players) {
      ...
      generate_plot(base_directory / player.name().directory_name() / Path::PlayerPlotsDirectoryName / Path::PlayerAveragePointsVsDateFileName);
      for (const GameCategory game_category : GameCategories) {
        ...
        generate_plot(base_directory / player.name().directory_name() / Path::PlayerPlotsDirectoryName / Path::player_place_percentage_vs_date_file_name(game_category));
      }
    }
